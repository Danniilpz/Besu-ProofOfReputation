...
public class RepuProposalRpcAcceptanceTest extends AcceptanceTestBase {
  ...
  public void shouldReturnProposals() throws IOException {
    ...
    minerNode1.verify(
        repu.proposalsEqual().addProposal(minerNode3).removeProposal(minerNode2).build());
    minerNode2.verify(repu.proposalsEqual().removeProposal(minerNode3).build());
    minerNode3.verify(repu.noProposals());
  }
}
