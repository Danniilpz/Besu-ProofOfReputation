...
/** Defines the protocol behaviours for a blockchain using Repu. */
public class RepuProtocolSchedule {
  ...
  public static ProtocolSchedule create(
    ...
    final RepuConfigOptions repuConfig = config.getRepuConfigOptions();

    final Address localNodeAddress = Util.publicKeyToAddress(nodeKey.getPublicKey());

    if (repuConfig.getEpochLength() <= 0) {
      throw new IllegalArgumentException("Epoch length in config must be greater than zero");
    }

    final EpochManager epochManager = new EpochManager(repuConfig.getEpochLength());
    ...
    return new ProtocolScheduleBuilder(
            ...
            ProtocolSpecAdapters.create(
                0,
                builder ->
                    applyRepuSpecificModifications(
                        epochManager,
                        repuConfig.getBlockPeriodSeconds(),
                        localNodeAddress,
                        builder,
  ...
  private static ProtocolSpecBuilder applyRepuSpecificModifications(
  ...
  private static BlockHeaderValidator.Builder getBlockHeaderValidator(
    ...
    return BlockHeaderValidationRulesetFactory.repuBlockHeaderValidator(
