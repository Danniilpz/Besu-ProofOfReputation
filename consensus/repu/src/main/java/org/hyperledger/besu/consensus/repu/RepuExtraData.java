 ...
 * operating under an Repu consensus mechanism.
 */
public class RepuExtraData implements ParsedExtraData {
  private static final Logger LOG = LoggerFactory.getLogger(RepuExtraData.class);
  ...
  public RepuExtraData(
    ...
    proposerAddress =
        Suppliers.memoize(() -> RepuBlockHashing.recoverProposerAddress(header, this));
  }

  public static Bytes createWithoutProposerSeal(
      final Bytes vanityData, final List<Address> validators) {
    return RepuExtraData.encodeUnsealed(vanityData, validators);
  }

  public static RepuExtraData decode(final BlockHeader header) {
    final Object inputExtraData = header.getParsedExtraData();
    if (inputExtraData instanceof RepuExtraData) {
      return (RepuExtraData) inputExtraData;
    }
    LOG.warn(
        "Expected a RepuExtraData instance but got {}. Reparsing required.",
        inputExtraData != null ? inputExtraData.getClass().getName() : "null");
    return decodeRaw(header);
  }

  static RepuExtraData decodeRaw(final BlockHeader header) {
    final Bytes input = header.getExtraData();
    if (input.size() < EXTRA_VANITY_LENGTH + SECPSignature.BYTES_REQUIRED) {
      throw new IllegalArgumentException(
          "Invalid Bytes supplied - too short to produce a valid Repu Extra Data object.");
    }
    ...
    return new RepuExtraData(vanityData, proposerSeal, validators, header);
  }
  ...
  public static String createGenesisExtraDataString(final List<Address> validators) {
    return RepuExtraData.createWithoutProposerSeal(Bytes.wrap(new byte[32]), validators)
        .toString();
  }
}
