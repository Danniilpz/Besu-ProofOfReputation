...
import org.hyperledger.besu.consensus.repu.*;
...
public class RepuBlockCreatorTest {
  ...
  public void setup() {
    ...
    protocolContext = new ProtocolContext(blockchain, stateArchive, repuContext);
  ...
  public void proposerAddressCanBeExtractFromAConstructedBlock() {
    ...
    final Bytes extraData =
        RepuExtraData.createWithoutProposerSeal(Bytes.wrap(new byte[32]), validatorList);

    final Address coinbase = AddressHelpers.ofValue(1);
    final RepuBlockCreator blockCreator =
  ...
  public void insertsValidVoteIntoConstructedBlock() {
    ...
    assertThat(createdBlock.getHeader().getNonce()).isEqualTo(RepuBlockInterface.ADD_NONCE);
  ...
  public void insertsNoVoteWhenAtEpoch() {
    ...
    final Block createdBlock = blockCreator.createBlock(0L).getBlock();
    assertThat(createdBlock.getHeader().getNonce()).isEqualTo(RepuBlockInterface.DROP_NONCE);
