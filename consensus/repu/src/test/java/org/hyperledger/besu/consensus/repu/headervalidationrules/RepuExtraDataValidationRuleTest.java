...
public class RepuExtraDataValidationRuleTest {
  ...
  public void missingSignerFailsValidation() {
    final Bytes extraData =
        RepuExtraData.createWithoutProposerSeal(Bytes.wrap(new byte[32]), Lists.newArrayList());
    ...
    final RepuExtraDataValidationRule rule =
        new RepuExtraDataValidationRule(new EpochManager(10));

    assertThat(rule.validate(child, parent, repuProtocolContext)).isFalse();
  }
  ...
  public void signerNotInExpectedValidatorsFailsValidation() {
    ...
    final BlockHeader badlySignedChild =
        TestHelpers.createRepuSignedBlockHeader(headerBuilder, otherSigner, Lists.newArrayList());

    final RepuExtraDataValidationRule rule =
        new RepuExtraDataValidationRule(new EpochManager(10));
    assertThat(rule.validate(badlySignedChild, parent, repuProtocolContext)).isFalse();
  }
  ...
  public void signerIsInValidatorsAndValidatorsNotPresentWhenNotEpochIsSuccessful() {
    ...
    final RepuExtraDataValidationRule rule =
        new RepuExtraDataValidationRule(new EpochManager(10));
    assertThat(rule.validate(correctlySignedChild, parent, repuProtocolContext)).isTrue();
  }
  ...
  public void epochBlockContainsSameValidatorsAsContextIsSuccessful() {
    ...
    final BlockHeader correctlySignedChild =
        TestHelpers.createRepuSignedBlockHeader(headerBuilder, proposerKeyPair, validatorList);
  ...
  public void epochBlockWithMisMatchingListOfValidatorsFailsValidation() {
    ...
    final RepuExtraDataValidationRule rule =
        new RepuExtraDataValidationRule(new EpochManager(10));
    assertThat(rule.validate(correctlySignedChild, parent, repuProtocolContext)).isFalse();
  }
